...
function addRow() {
  ...
  //     newRow +=
  //       "<option value='" + payers[i].name + "'>" + payers[i].name + "</option>";
  //   }

  // Complete the HTML string for the new row
  //   +"<td><input type='text' class='form-control-sm' name='amount.' form='expenseForm' size='10' placeholder='$' maxlength='10' required pattern='(?=.*?\\d)^(([1-9]\\d{0,2}(\\d{3})*)|\\d+)?(\\.\\d{1,2})?$' title='Format must be currency value without dollar sign or commas e.g. 1, 2.50, 1500.75'></td></tr>";
...
function updateTableElements(rowCount) {
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    table.children[1].children[i].children[0].innerHTML =
    ...
    for (let j = 0; j < 5; j++) {
      ...
      table.children[1].children[i].children[
        j + 1
      ].firstElementChild.setAttribute("name", newName);
    }
  }
}
